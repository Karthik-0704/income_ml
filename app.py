...
if TRACKING_URI and DAGSHUB_TOKEN:
    ...
    print("Warning: MLflow credentials not found in environment.")
...
import pickle # Add this to your imports at the top
...
def load_specific_model(model_key: str):
    ...
    try:
        ...
        # This points to the folder where MLflow will download the artifact
        dest_folder = os.path.join(LOCAL_STORAGE, model_key)
        # This points to the specific file inside that folder
        pkl_file_path = os.path.join(dest_folder, "model", "model.pkl")

        # 1. Download the 'model' artifact folder from DagsHub
        ...
        # 2. Load using standard pickle instead of mlflow.pyfunc
